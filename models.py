...
class Room(db.Model):
    ...
    cleaning_status = db.Column(db.String(20), default='clean')
    
    # Changed from ENUM to String for MySQL compatibility
    status = db.Column(db.String(20), default='available')
    
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    last_cleaned = db.Column(db.DateTime)
    last_occupied = db.Column(db.Date)
...
class Expense(db.Model):
    ...
    amount = db.Column(db.DECIMAL(10, 2), nullable=False)  # Changed to DECIMAL
...
class Booking(db.Model):
    ...
    total_amount = db.Column(db.DECIMAL(10, 2), nullable=True)  # Changed to DECIMAL
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    ...
    booking_source = db.Column(db.String(50), default='direct')
    ...
    def generate_checkout_otp(self):
        self.checkout_otp = secrets.randbelow(1000000)
        self.checkout_otp = str(self.checkout_otp).zfill(6)
...
class BarItem(db.Model):
    ...
    name = db.Column(db.String(100), nullable=False)
    description = db.Column(db.Text)
    price = db.Column(db.DECIMAL(10, 2), nullable=False)  # Changed to DECIMAL
...
class BarSale(db.Model):
    ...
    total_amount = db.Column(db.DECIMAL(10, 2), nullable=False)  # Changed to DECIMAL
...
class BarOrder(db.Model):
    ...
    user = db.relationship('User', backref='bar_orders')
...
class OrderItem(db.Model):
    ...
    price = db.Column(db.DECIMAL(10, 2))  # Changed to DECIMAL
...
class CleaningAssignment(db.Model):
    ...
    cleaning_log = db.relationship(
        'CleaningLog', 
        backref=db.backref('cleaning_assignment', uselist=False),
        uselist=False, 
        lazy=True
    )
...
class CleaningLog(db.Model):
    ...
    staff_id = db.Column(db.Integer, db.ForeignKey('staff.id'))
    quality_checker_id = db.Column(db.Integer, db.ForeignKey('staff.id'))  # Fixed duplicate column
...
class StoreInventory(db.Model):
    ...
    unit_cost = db.Column(db.DECIMAL(10, 2), nullable=False)  # Changed to DECIMAL
